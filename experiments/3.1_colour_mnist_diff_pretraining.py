...
EXPERIMENT_DIR = ROOT / Path(f"artifacts/experiment_results/VaryPretraining_ColourMNIST_source{SOURCE_THETA}_target{TARGET_THETA}_eval{EVAL_THETA}")
...
def pretrain_MNIST_models(
    ...
    for run in trange(NUMBER_RUNS, desc="Degraded Pretraining Runs"):
        ...
        num_epochs = 0
        model_files = []
        for epochs in PRETRAINING_EPOCHS:
            num_epochs = epochs - num_epochs
            net.partial_fit(train_dataset, y=None, epochs=num_epochs)
        ...
        # Save model
        list_of_model_files.append(model_files)
...
def train_MNIST_model_from_pretrained_init(
    ...
    for epoch, fp in pretrained_weights_fp:
        ...
        net.initialize()
        print(f"Loading from {str(fp)}")
        state_dict = torch.load(str(fp), map_location=net.device)
